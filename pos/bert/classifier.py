...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,labels, output_mask,num_labels):
    ...
    with tf.variable_scope("loss"):
        ...
        one_hot_labels = tf.one_hot(labels, depth=num_labels, dtype=tf.float32,axis = -1) #batch_size,seq_length,depth

        per_example_loss = -tf.reduce_sum(one_hot_labels * log_probs, axis=-1) #batch_size,seq_length
        per_example_loss=per_example_loss*tf.cast(x=output_mask,dtype=tf.float32)
        loss = tf.reduce_mean(per_example_loss)
        
        pre_label=tf.argmax(probabilities,axis=-1,name='label_prediction')#batch_size,seq_length
        pre_label=tf.cast(pre_label,dtype=tf.int32)*tf.cast(output_mask,dtype=tf.int32)
